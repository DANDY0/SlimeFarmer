...
public abstract class SoundManagerBase : Singleton<SoundManager>
{
	...
	[SerializeField, ReadOnly] protected AudioSource m_BiomSource;
	[SerializeField, ReadOnly] protected AudioSource m_UiSoundSource;
	
	[SerializeField] private AudioClip[] m_SlimeCatch;
	...
	protected virtual void setRefs()
    {
		...
		m_BiomSource = transform.FindDeepChild<AudioSource>("Biom Source");
	...
	public virtual void PlaySlimeCatch()
	{
		if(IsSFXMuted)
			return;
		var index = Random.Range(0, m_SlimeCatch.Length);
		m_BiomSource.PlayOneShot(m_SlimeCatch[index], m_StorageManager.Sound.Value);
	}
