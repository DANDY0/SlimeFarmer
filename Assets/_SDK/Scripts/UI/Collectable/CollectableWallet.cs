...
public class CollectableWallet : MonoBehaviour
{
    ...
    {
        if (CollectableType == eCollectable.UniqueSlimes)
        {
            m_AmountDisplay.text = NumberFormatter.GetFormatedNumber(amount) 
                                   + "/" + GameplayVariables.MaxDiscoveredSlimesCount[PlayerPrefs.GetInt(c_PortalsBought)];
            return;
        }
        m_AmountDisplay.text = NumberFormatter.GetFormatedNumber(amount);
    ...
    public void Add(int amount, bool isNeededAnimation = true)
    {
        ...
        {
            m_PunchingIcon?.Kill();
            m_TargetTransform.localScale = Vector3.one * .9f;
